...
fn main() {
    ...
    // fn main() {
    // Create a channel with a transmitting (tx) and receiving (rx) end.
    let (tx, rx) = channel::unbounded();

    // Spawn the first child thread
    let rx1 = rx.clone(); // Clone the receiver for the second child thread
    let child1 = thread::spawn(move || {
        while let Ok(value) = rx1.recv() {
            println!("Child 1 received: {}", value);
        }
        println!("Child 1: Channel closed, exiting.");
    });

    // Spawn the second child thread
    let child2 = thread::spawn(move || {
        while let Ok(value) = rx.recv() {
            println!("Child 2 received: {}", value);
        }
        println!("Child 2: Channel closed, exiting.");
    });

    // Main thread: send several values and print them
    for i in 1..6 {
        println!("Main thread sending: {}", i);
        tx.send(i).expect("Failed to send value");
        thread::sleep(Duration::from_millis(500)); // Sleep to simulate work
    }

    // Close the sending end by dropping `tx`
    drop(tx);
    println!("Main thread: Closed channel.");

    // Join the child threads
    child1.join().expect("Child 1 panicked");
    child2.join().expect("Child 2 panicked");

    println!("Main thread: Both child threads have exited.");
    // }
