...
fn main() {
    ...
    let result;
    {
        ...
        // `result` now holds a reference to one of the inputs, either `string1` or `string2`
    }
    println!("The longest string is {}", result); // ERROR: `result` may point to `string2`, which is now out of scope
}
